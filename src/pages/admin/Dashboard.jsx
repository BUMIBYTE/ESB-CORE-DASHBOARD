...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const [cpuRes, memRes, serverRes, jobRes] = await Promise.all([
      ...
      setStats([
        ...
        { 
          label: "Network Peak", 
          value: server.networkPeak || "0 Mbps", 
        ...
        { 
          ...
          subValue: `${jobList.length} Total registered`,
                ...
                {jobs.length > 0 ? jobs.map(j => (
                  <tr key={j.id} className="hover:bg-slate-50/50 transition-all group">
                    ...
                    <td className="px-8 py-5 text-center">
                      <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider border shadow-sm transition-all ${
                        j.status === "running" ? "bg-emerald-50 text-emerald-600 border-emerald-100 group-hover:bg-emerald-100" :
